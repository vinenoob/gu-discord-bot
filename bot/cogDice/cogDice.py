...
class Dice(commands.Cog):
	...
	async def roll(self, ctx, *, QuantityDSides):
	    ...
	    if Invalid: #this should only come up if the numbers might actually hurt senpai (or if there is no d)
		    await ctx.send("'{}'? You are objectively wrong. Use '!roll xDy'. \n x = Number of dice, \n y = Dice sides. \n ie: '!roll 2d6'.".format(QuantityDSides))
	    ...
	    if Quantity > 10000:
		    await ctx.send("I'm not rolling that many dice, it would take *forever*.")
		    return

	    #actually roll the dice
	    await ctx.send("Rolling " + str(Quantity)+ " " +str(Sides) + " sided dice.")
	    ...
	    await ctx.send("Rolled a " + str(Total))